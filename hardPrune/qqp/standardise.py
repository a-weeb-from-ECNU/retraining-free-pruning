...
from .qqp_prune_model import getPrunedModel
from transformers.models.bert.modeling_bert import BertEmbeddings

class StandardBertSelfAttention(nn.Module):
    """标准BERT自注意力层，适配剪枝后的不同注意力维度"""
...
def extract_layer_configs_from_pruned_model(pruned_model):
    ...
    for i, layer in enumerate(pruned_model.layers):
        ...
        print(f"Layer {i}: attention_dim={attention_dim}, intermediate_size={intermediate_size}")
...
def convert_pruned_to_standard_bert(pruned_model, config=None):
    ...
    Args:
        pruned_model: 你的PrunedBertForSequenceClassification模型
        config: BERT配置
...
def transfer_weights_from_pruned(pruned_model, standard_model):
    ...
    for i, (pruned_layer, standard_layer) in enumerate(zip(pruned_model.layers, standard_model.bert.encoder.layer)):
        ...
        print(f"✓ Layer {i}权重迁移完成 (attention_dim: {pruned_layer.query.out_features}, intermediate_size: {pruned_layer.intermediate.out_features})")
...
def verify_model_structure(model):
    ...
    for module_name in expected_modules:
        if hasattr(model, module_name):
            ...
            print(f"✗ 缺少 {module_name}")
    
    # 检查bert模块结构
    if hasattr(model, 'bert'):
        bert_modules = ['embeddings', 'encoder', 'pooler']
        for module_name in bert_modules:
            if hasattr(model.bert, module_name):
                print(f"✓ bert.{module_name}")
            else:
                print(f"✗ 缺少 bert.{module_name}")
    ...
    if hasattr(model.bert, 'encoder') and hasattr(model.bert.encoder, 'layer'):
        ...
        if num_layers > 0:
            ...
            for module_name in layer_modules:
                if hasattr(first_layer, module_name):
...
def getStandardModel():
    ...
    # 验证转换结果
    # verify_model_structure(standard_model)
