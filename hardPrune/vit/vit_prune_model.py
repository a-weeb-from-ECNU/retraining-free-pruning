...
def load_and_transfer_weights(model, state_dict, head_mask, neuron_mask):
    ...
    for layer_idx in range(len(model.vit.encoder.layer)):
        ...
        print(f"Layer {layer_idx}: 保留 {len(keep_heads)}/{num_heads} 个注意力头, {len(keep_neurons)} 个MLP神经元")
        print(f"  头缩放因子: {head_scale_factors.tolist()}")
        print(f"  神经元缩放因子范围: [{neuron_scale_factors.min():.3f}, {neuron_scale_factors.max():.3f}]")
        ...
        for weight_type in ['query', 'key', 'value']:
            ...
            if old_key in state_dict:
                ...
                print(f"目标层权重形状: {target_param.shape}")
        ...
        if old_key in state_dict:
            ...
            print(f"Layer {layer_idx} 注意力输出权重: 原始={old_weight.shape}, 剪枝后={new_weight.shape}, 目标={target_param.shape}")
            
            if new_weight.shape == target_param.shape:
                target_param.data.copy_(new_weight)
            else:
                print(f"警告: Layer {layer_idx} 注意力输出权重形状不匹配")
                print(f"  预期: {target_param.shape}, 实际: {new_weight.shape}")
        ...
        if old_key in state_dict:
            ...
            if old_bias.shape == target_param.shape:
                ...
                print(f"警告: Layer {layer_idx} 注意力输出偏置形状不匹配")
    ...
    for norm_param in ['weight', 'bias']:
        ...
        if old_key in state_dict:
            ...
            if state_dict[old_key].shape == target_param.shape:
                ...
                print(f"转移权重: {old_key} -> vit.layernorm.{norm_param}")
...
def getPrunedViTModel():
    try:
        ...
        for i, layer in enumerate(model.vit.encoder.layer):
            print(f"Layer {i} - Num Attention Heads: {layer.attention.attention.num_attention_heads}")
            print(f"Layer {i} - MLP Hidden Units: {layer.intermediate.dense.out_features}")
